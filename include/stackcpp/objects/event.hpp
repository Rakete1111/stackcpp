...
#ifndef STACKCPP_EVENT_HPP
#define STACKCPP_EVENT_HPP
...
inline namespace objects { struct event; }
event parse_event(const rapidjson::Value&);
...
struct event final {
    ...
    event_type type() const noexcept { return type_; }
    std::string excerpt() const noexcept { return excerpt_; }
    std::string url() const noexcept { return url_; }

private:
    friend event stackcpp::parse_event(const rapidjson::Value&);
